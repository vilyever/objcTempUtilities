...
static id aspect_add(id self, SEL selector, AspectOptions options, id block, NSError **error) {
    ...
    aspect_performLocked(^{
        // 检测方法是否可以hook，检测父类是否已经 hook当前方法
        if (aspect_isSelectorAllowedAndTrack(self, selector, options, error)) {
            // 获取当前self，sel的容器（包含三种位置的hook方法集）
            AspectsContainer *aspectContainer = aspect_getContainerForObject(self, selector);
            // 生成identifier
            identifier = [AspectIdentifier identifierWithSelector:selector object:self options:options block:block error:error];
            if (identifier) {
                // 向Container添加此次hook的信息
...
static Class aspect_hookClass(NSObject *self, NSError **error) {
	...
	if ([className hasSuffix:AspectsSubclassSuffix]) {
        ...
        // 当前正在hook类，即该类的所有instance都会被 hook
    ...
    // 创建一个子类，类名为当前类（注意，此时传入的self是instance而非class，因baseClass不是MetaClass）加上后缀
	...
	if (subclass == nil) {
        ...
        // 替换subclass的metaClass的class方法，使之返回当前instance的class
...
static BOOL aspect_isSelectorAllowedAndTrack(NSObject *self, SEL selector, AspectOptions options, NSError **error) {
    ...
    // Additional checks.
    // 因为DEALLOC只能前向hook，检测传入的hook位置是否正确
    ...
    if (class_isMetaClass(object_getClass(self))) {
        ...
        do {
            ...
            if ([tracker.selectorNames containsObject:selectorName]) {
                ...
                // 找出最上层的tracker类，提示已在最上层tracker类中hook
                if (tracker.parentEntry) {
                    ...
                    // 当前类就是最上层的类（NSOBJECT?)，因为已经为当前类hook过，无须添加tracker信息，直接return
        ...
        // 为包括当前类的所有父类添加tracker，且为所有tracker添加sel已hook信息
        do {
            AspectTracker *tracker = swizzledClassesDict[currentClass];
            if (!tracker) {
